...
def get_text(lang, key):
    texts = {
        'ru': {
            ...
            'fallback_search': "Также ты можешь сам найти вакансии по ссылкам:\n🔗 [hh.ru](https://hh.ru/search/vacancy?text={query}&experience=noExperience)\n🔗 [Работа.ру](https://www.rabota.ru/vacancy/?query={query})"
        },
        'en': {
            ...
            'fallback_search': "You can also search for vacancies yourself using these links:\n🔗 [hh.ru](https://hh.ru/search/vacancy?text={query}&experience=noExperience)\n🔗 [Rabota.ru](https://www.rabota.ru/vacancy/?query={query})"
        }
    }
...
async def analyze_career_async(age: int, interests: str, education: str, skills: str, language: str) -> Tuple[str, str]:
    if language == 'ru':
...
def find_vacancies(skills: str, interests: str) -> List[Dict]:
    """Ищет вакансии на hh.ru для новичков, возвращает список."""
    # Формируем поисковый запрос: добавляем ключевые слова для начинающих
    query_parts = [skills, interests, "стажировка", "junior", "без опыта", "студент"]
    ...
    params = {
        "text": query,
        "area": 113,                # Россия
        "per_page": 20,             # Больше результатов
    ...
    try:
        ...
        for item in data.get("items", []):
            # Дополнительная фильтрация: оставляем только подходящие по названию
            name = item.get("name", "").lower()
            if any(kw in name for kw in ["стажёр", "intern", "junior", "trainee", "младший", "помощник"]):
...
async def diagnostic_interests(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def diagnostic_education(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def diagnostic_skills(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def cancel_diagnostic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    lang = get_user_language(user_id)
...
async def search_jobs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    vacancies = find_vacancies(user_data['skills'], user_data['interests'])

    if not vacancies:
        # Если не найдено, предлагаем ссылки на поиск вручную
        query = f"{user_data['skills']} {user_data['interests']}".replace(" ", "+")
        fallback = get_text(lang, 'fallback_search').format(query=query)
        await update.message.reply_text(
